...
基于 yt-dlp 的 Windows 桌面工具，支持单个视频、播放列表、频道下载，带 GUI 与依赖自动安装。可以自动剪辑yotube视频将你需要的时间段剪切出来

## 本地运行

1. 安装 Python 3.10+
2. 双击 `setup.bat` 安装依赖
3. 双击 `打开YouTube下载器.bat` 启动
...
3. 创建 Release 并上传 `YouTubeDownloader.exe`
...
2. 修复代码或 `.github/workflows/release.yml` 问题
3. 删除失败的 tag 并重新创建：
...
- 仓库需位于 `secure-artifacts` 组织下
- CI 配置文件：`.github/workflows/release.yml`
- 发布产物由 `github-actions[bot]` 自动上传，请勿在 GitHub 网页手动上传 Release 文件
